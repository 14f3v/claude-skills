...
- Device cert **expired** (90-day branch role TTL) → **operator re-enrollment, no way around it.** Auto-renewal (live 2026-08-06) prevents this but cannot cure it: the cert IS the credential for `/renew`, so an expired one is refused at the relay handshake, by the signer (`401 cert_expired`), and by the gateway. AppGate routes to the enrollment screen (`!enrolled || expired`); operator mints a fresh token. After re-enroll (new serial, same CN) the app must null `_cachedContext` + reinstall `HttpOverrides.global` or restart. (See `mjbl-client-provisioning`.)
- **Device is inside its 30-day window but never renews** → check, in order: (1) is it on an APK with `MTLS_RENEW_BASE` set? A tag build with an empty default ships internal-only renewal and fails silently off-site — check the build log's `renewal endpoint:` line; (2) `curl -o /dev/null -w '%{http_code}' https://microloan.maruhanjapanbanklao.com:2401/renew` from **off-network** → expect **401**; (3) signer audit for `renew outcome=denied` + `reason` (`too_early` >60d left, `cert_revoked`, `cn_mismatch`, `branch_not_allowed`); (4) the per-CN limiter is **3/min** — back-to-back manual tests trip a clean `429`. Renewal is fire-and-forget and swallows its own errors, so the device shows nothing: the signer audit log is the only place the truth appears.
- **`/renew` works internally but not publicly** → the relay cert is missing the `microloan.maruhanjapanbanklao.com` SAN. TLS terminates at the RELAY for public renewals (the gateway `:2401` is a non-decrypting passthrough), so the device validates that name against the *relay's* cert, not the gateway's. `/enroll` stays healthy throughout, which makes this look like a routing fault when it is a SAN fault.
