...
- **Endpoints**: `POST /sign` (called by relay; unauthenticated at app layer — gated by source IP + token), **`POST /renew`**, `POST /mint`, `POST /revoke`, `GET /devices`, `GET /allowlist` + `POST /allowlist` + `DELETE /allowlist/<code>`, `GET /activity`, `GET /claim-status?claim_id=…`, `GET /healthz`. All except `/sign`, `/renew` and `/healthz` require `Authorization: Bearer <admin_token>`.
- **`POST /renew` (token-free, live 2026-08-06)** — the device's CURRENT valid cert IS the credential. The relay forwards it as base64 DER in `X-Client-Cert-DER`; the signer **re-validates independently** and **fails closed at 8 stages, in this order**: reject a `token` field → decode header → chain-verify (`openssl verify -purpose sslclient -no_check_time`) → expiry / `too_early` (a cert with >60d left is refused: `RENEW_MAX_REMAINING_SECONDS`) → CN split → **live allowlist re-read** → **Vault revocation gate** → CSR proof-of-possession + CN equality. Emits `renew outcome=ok` carrying `prev_serial` / `prev_expires_at`.
  - 🚨 **The revocation gate is the whole security argument.** Python `ssl` performs **no CRL/OCSP check**, so a revoked-but-unexpired cert completes the relay handshake perfectly. `vault_cert_status()` is deliberately **tri-state** (`live`/`revoked`/`unknown`) and denies on BOTH `revoked` and `unknown` — the older `vault_cert_revocation_time()` returned `None` for a live cert *and* for 404/non-200/unparseable, which is correct fail-soft for `/devices` but **fails OPEN** as a gate.
  - `/renew` does **not** revoke the superseded cert (brick risk); the compensating control is that revoke-by-branch+uuid revokes **every** unexpired serial, scanning both `sign` AND `renew` audit events.
...
- Endpoints: `POST /enroll` (forwards body byte-identical to signer `/sign`), `POST /renew`, `GET/HEAD /healthz`. Env: `SIGNER_URL=https://10.88.1.116:8444`, `SIGNER_CA=/tls/mjbl-root.crt`, `TLS_CERT=/tls/tls.crt`, `TLS_KEY=/tls/tls.key`, `LISTEN_PORT=8443`, `PUBLIC_RENEW_PORT=8445`, `RENEW_ENABLED=1`, `RATE_LIMIT_PER_MIN=30` (per src-ip+branch, in-memory per-pod), `RENEW_RATE_LIMIT_PER_MIN=3` (per **verified CN**), `SIGNER_TIMEOUT=15`.
- **TWO listeners on one TLS context (`0.4.0`+).** `LISTEN_PORT=8443` = the full internal surface (`/enroll`, `/renew`, `/healthz`). `PUBLIC_RENEW_PORT=8445` = **`/renew` ONLY** — this is the socket the gateway's `:2401` passthrough targets and therefore the one exposed to the internet.
  - The routing decision is **which local socket the connection landed on** (`self.connection.getsockname()[1]`), checked as the first statement of `do_GET`/`do_HEAD`/`do_POST`. Server-side and unspoofable — unlike `Host` or `X-Forwarded-*`, which the client controls. **The request path lives inside TLS**, so no NAT/LB/SNI layer in front could ever expose `/renew` without also exposing `/enroll`.
  - `PUBLIC_ALLOWED_PATHS = frozenset({"/renew"})` — an **allowlist**, because a denylist silently leaks every path added later. Off-list → **404 `not_found`**, not 403 (a 403 confirms the endpoint exists). Query strings are stripped before matching, so `/enroll?x=1` cannot smuggle through.
  - `getsockname()` raising **fails CLOSED** (treated as public). `validate_listener_ports()` refuses startup when public == internal, which would otherwise apply renew-only policy to `/enroll` fleet-wide.
  - `version_string()` returns `""` (`0.4.1`) — the default `Server: mjbl-enroll-relay/0.1.0 Python/3.12.13` was fine internally but is free reconnaissance on a public endpoint.
  - Covered by `tests/test_public_listener.py` (13 tests: allowlist-not-denylist, case/query/traversal smuggling, port collision, `Server` disclosure).
...
- Service is a MetalLB LoadBalancer exposing **both** ports: `8443` (`https`) and `8445` (`renew-only`).
- Server cert: `enroll-relay-tls` Secret (keys `tls.crt`, `tls.key`, `mjbl-root.crt`), serverAuth, **RSA-2048**, SANs `enroll.vte.mjblao.local` + `enroll.maruhanjapanbanklao.com` + **`microloan.maruhanjapanbanklao.com`** + IP `10.88.101.143`, signed from `mjbl-platform-role`. Bootstrap/externally-managed (NOT in git, NOT kustomize-tracked). The `microloan` SAN was added 2026-08-07 because TLS terminates at the RELAY for public renewals — the device connects to `microloan.…:2401` and validates that name against **this** cert, not the gateway's.
- Image `mjcr.vte.mjblao.local/ghcr/mjbl-digital/enroll-relay:0.4.1` (`imagePullPolicy: IfNotPresent` — bump the tag on rebuild, never re-push the same tag). A `v*` tag on the repo builds, pushes, and **auto-bumps this tag in `k8s-config`** → ArgoCD rolls it. Tagging is a production write.
...
**Exercise `/renew` locally — `tools/devstack/`** (in the `mjbl-mtls-enrollment` repo). Runs the **real** `relay/app.py` + `signer/mjbl_enroll_signer.py` unmodified (env-driven) against a throwaway Vault in Docker with its own CA. Production cannot be used to test renewal on demand for two reasons: `RENEW_MAX_REMAINING_SECONDS` (60d) rejects a fresh cert as `too_early`, and prod Vault binds `127.0.0.1:8200`. The dev stack raises the floor to 100 days so a new 90-day cert is renewable immediately.
```bash
export REPO=<checkout> DEVSTACK=/tmp/mjbl-devstack     # runtime dir OUTSIDE the repo — holds real keys
./tools/devstack/bootstrap.sh                          # vault + PKI + roles + approle + TLS
```
- **The intermediate MUST mount at `pki/`** — the signer hardcodes `pki/sign/…`, `pki/revoke`, `pki/crl/rotate`. Prod has the same shape.
- `sign-intermediate` with `format=pem_bundle` returns **cert + root**; take only the first PEM block or the CA bundle carries a duplicate root.
- Drive the client from a **host-side `flutter test`**, not an emulator: the Dart VM has full `dart:io` (real `SecurityContext`, real TLS) and `debugPrint` actually reaches you — in a release APK it does not reach logcat. **Call `MtlsBootstrap.initialize()` first** or the POST goes out with no client cert and surfaces as a baffling empty `400` that looks like a server bug.
- Tear down with `shred -u`, not `rm` — `$DEVSTACK` holds private keys. Full detail in `tools/devstack/README.md`.
