...
| `MTLS_RENEW_BASE` | `https://microloan.maruhanjapanbanklao.com:2401` | **PUBLIC renewal edge** (live 2026-08-07). Split from `enrollBase` in agency_v2#70 and defaults to it when unset — enrollment is an operator-attended, on-site act; renewal must work wherever the tablet is when its window opens. NAT → gateway `10.88.101.142:2401` → passthrough → relay `:8445`. |
...
**Renewal = unattended, token-free** (`MtlsRenewal`, live 2026-08-06; public edge 2026-08-07). `MtlsBootstrap.initialize()` fires `unawaited(MtlsRenewal.attemptRenew())` on **every app open** — fire-and-forget, so it never blocks start and never surfaces an error. The device generates a NEW keypair + CSR (CN parsed back out of the stored chain), POSTs to `${renewBase}/renew` **over mTLS presenting its current cert as the credential**, saves the identity, then hot-swaps the `SecurityContext`. Policy is pure and unit-tested in `shouldAttemptRenew()`:
- **`kRenewWindow` = 30 days**, and per-device jitter (FNV-1a of the UUID, 0–47h) is **SUBTRACTED** — a jittered device becomes eligible *later*. Adding it would pull the whole cohort forward and defeat the purpose. This matters because the fleet was batch-enrolled and expires in batches.
- **`kRenewCliff` = 7 days**: inside the cliff, exponential back-off is **ignored entirely** and it retries hourly. For a device with days rather than weeks left, that is the difference between renewing and needing a physical operator visit.
- **A null `expiresAt` is ELIGIBLE on purpose.** Devices enrolled before the expiry key existed report `expiryUnknown:true, renewSoon:true` — deliberately **not** `expired:true` (the cert may be fine, and a needless wipe costs an operator visit). That cohort used to read as perfectly healthy, so it showed no banner, was never prompted, and hard-expired silently. Renewal is the repair path: a successful renew backfills the metadata.
- One-deep identity **backup/restore** (`restoreBackupIfAny()`) so a partial multi-key write cannot brick the device.
...
- **🚨 A `v*` tag build receives NO `workflow_dispatch` inputs.** Every `github.event.inputs.*` is empty on a tag push, so the `|| 'default'` fallback in the workflow **is** the shipped contract. Tagging `v1.1.43` while `MTLS_RENEW_BASE`'s default was still empty would have produced versionCode **55** — superseding the good build on every pilot device and silently reverting the fleet to internal-only renewal, invisibly, since renewal is unattended. Flip the **workflow default** (agency_v2#75) and confirm it in the build log (`renewal endpoint: …`) before tagging.
- **Python heredocs at column 0 inside a `run: |` block terminate the YAML block scalar.** The whole workflow then parses as garbage — GitHub shows the *file path* as the workflow name and `workflow_dispatch` 422s. Put non-trivial logic in `.github/scripts/` and call it (agency_v2#74).
- **Firebase distributes to a group ALIAS, and distributing to a group that does not exist SUCCEEDS and reaches nobody.** Nothing records who is in `pilot`. `.github/workflows/firebase-testers-list.yml` (read-only, manual dispatch) answers it and exits non-zero on a missing/empty group. It lists *who can install*, not which cert cohort those devices are in — matching the two is still manual.
...
- **CN rebuild after renewal:** `_cachedContext` is immutable for the app lifetime; after a re-enroll (new serial, same CN) null it + re-read storage + re-install `HttpOverrides.global` (or restart). Auto-renewal does this hot-swap itself; the manual path (long-press the login version chip → Re-enroll) remains for expired devices.
- **An EXPIRED cert cannot renew itself — it is full operator re-enrollment, always.** For `/renew` the cert IS the credential, so an expired one is refused at the relay handshake, again by the signer (`401 cert_expired`), and again by the gateway. A grace period would mean accepting an expired credential as identity. Auto-renewal reduces how often this happens; it does not create a recovery path. `AppGate._check()` → `!enrolled || expired` → EnrollmentScreen.
- **Never wipe the identity on a TLS handshake failure.** `isMtlsHandshakeFailure()` deliberately only *records* (`certRejected`, session-scoped). A stale CRL bundle or a gateway misconfig rejects **every** device at once — there is precedent — and wiping on that signal would turn a ten-minute server-side fix into a fleet-wide re-enrollment, each device needing a fresh operator token. Detect and report; never destroy.
